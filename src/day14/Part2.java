...
public class Part2 {
    ...
    public Part2() throws FileNotFoundException {
        ...
        while (scanner.hasNextLine()) {
            ...
            pos.add(Arrays.stream(line.substring(line.indexOf("p=") + 2, line.indexOf(" ")).split(","))
                    .mapToInt(Integer::parseInt)
                    .toArray());
            vel.add(Arrays.stream(line.substring(line.indexOf("v=") + 2).split(","))
                    .mapToInt(Integer::parseInt)
                    .toArray());
        }
            ...
            for (int i = 0; i < height - 3; i++) {
                for (int j = 3; j < width - 3; j++) {
                    if (map.get(i).charAt(j) == '#'
                            && map.get(i + 1).charAt(j - 1) == '#' && map.get(i + 1).charAt(j + 1) == '#'
                            && map.get(i + 2).charAt(j - 2) == '#' && map.get(i + 2).charAt(j + 2) == '#'
                            && map.get(i + 3).charAt(j - 3) == '#' && map.get(i + 3).charAt(j + 3) == '#') {
                        display = true;
                    }
                }
            }
